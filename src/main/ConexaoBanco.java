...
public class ConexaoBanco {
	...
	public static void atualizarMorador() throws SQLException {
	    ...
	    while (true) {
	        ...
	        try (Connection conexao = conexao_com_banco(); PreparedStatement stmt = conexao.prepareStatement(verificarIdSQL)) {
	...
	public static void deletarMorador() throws SQLException {
	    ...
	    while (true) {
	        ...
	        try {
	            idMorador = Integer.parseInt(scanner.nextLine());
	            if (idMorador <= 0) {
	                System.out.println("\033[31mID deve ser um número positivo válido. Por favor, tente novamente.\033[0m");
	                continue;
	            }
	        } catch (NumberFormatException e) {
	            System.out.println("\033[31mID inválido! Por favor, insira um número válido.\033[0m");
	...
	public static void atualizarReserva() throws SQLException {
	    ...
	    while (true) {
	        ...
	        try (Connection conexao = conexao_com_banco(); PreparedStatement stmt = conexao.prepareStatement(verificarIdSQL)) {
	            ...
	            ResultSet rs = stmt.executeQuery();
	            if (rs.next() && rs.getInt(1) == 0) {
	                System.out.println("\033[31mNenhuma reserva encontrada com o ID informado. Tente novamente.\033[0m");
	                continue; // Se não encontrar o ID, solicita novo ID
	            }
	        } catch (SQLException e) {
	    ...
	    try (Connection conexao = conexao_com_banco(); PreparedStatement cursor = conexao.prepareStatement(sql)) {
	        ...
	        String statusReserva = "Confirmada"; // Ou você pode modificar conforme a lógica de status de reserva que precisar
	...
	public static void inserirPagamento() throws SQLException {
	    ...
	    while (true) {
	        ...
	        try (Connection conexao = conexao_com_banco(); PreparedStatement stmt = conexao.prepareStatement(verificarReservaSQL)) {
	            stmt.setInt(1, idReserva);
	            ResultSet rs = stmt.executeQuery();
	            if (rs.next() && rs.getInt(1) == 0) {
	                System.out.println("\033[31mNenhuma reserva encontrada com o ID informado. Tente novamente.\033[0m");
	...
	public static void atualizarPagamento() throws SQLException {
	    ...
	    while (true) {
	        ...
	        try (Connection conexao = conexao_com_banco(); PreparedStatement stmt = conexao.prepareStatement(verificarPagamentoSQL)) {
	            stmt.setInt(1, idPagamento);
	            ResultSet rs = stmt.executeQuery();
	            if (rs.next() && rs.getInt(1) == 0) {
	                System.out.println("\033[31mNenhum pagamento encontrado com o ID informado. Tente novamente.\033[0m");
	                continue; // Se não encontrar o ID, solicita novo ID
	            }
	        } catch (SQLException e) {
	...
	public static void main(String[] args) throws SQLException {
		...
		while (true) {
			...
			System.out.println(verde + "Bem-vindo ao Sistema de Locação de Condomínio. Escolha uma opção:\n"
